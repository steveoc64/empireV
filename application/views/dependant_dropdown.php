...
$(document).ready(function() {
	...
	$('#role_input_box').append('<img src="http://15mm-madness.com/empire//images/ajax-loader.gif" border="0" id="role_ajax_loader" class="dd_ajax_loader" style="display: none;">');
	var commander_id = $('select[name="commander_id"]');
	role.change(function() {var select_value = this.value;
	$('#role_ajax_loader').show();
	commander_id.find('option').remove();
	var myOptions = "";
	$.getJSON('http://15mm-madness.com/empire/user_account/get_commands/'+select_value, function(data) {
		commander_id.append('<option value=""></option>');
		$.each(data, function(key, val) {
			commander_id.append($('<option></option>').val(val.value).html(val.property));
		});
		$('#commander_id_input_box').show();
role.each(function(){$(this).trigger("liszt:updated");});
commander_id.each(function(){$(this).trigger("liszt:updated");});
$('#role_ajax_loader').hide();
});});});
</script>
